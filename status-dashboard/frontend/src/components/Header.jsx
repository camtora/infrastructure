export function Header({ status, lastUpdate, adminAuth, onEmergencyClick, apiKeyActive, onAskClick, onDnsClick, dnsOpen }) {
      ...
      <div class="grid grid-cols-3 items-center gap-4">
        ...
        <div class="flex flex-col items-center gap-1">
          <div class="flex items-center gap-2.5">
            <span class={`w-3 h-3 rounded-full flex-shrink-0 ${config.bg} ${config.pulse} ${config.glow}`} />
            <p class={`text-sm font-medium ${config.color}`}>{config.label}</p>
          </div>
        </div>

        {/* Right — buttons + timestamp */}
          ...
          <div class="flex items-center gap-3">
            ...
            {apiKeyActive ? (
              ...
              <div class={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${
                adminAuth.cached ? 'bg-amber-500/10 border-amber-500/20' : 'bg-violet-500/10 border-violet-500/20'
